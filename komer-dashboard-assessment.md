...
The payout, refund, and PIN paths do read-then-write without atomicity or idempotency keys. The **PIN brute-force window is concrete and route-level**: the withdrawal PIN attempt counter is a non-atomic read-modify-write, so parallel wrong guesses defeat the lockout regardless of the backend. The **double-spend risk is conditional**: the payout checks balance and then calls the edge function without a transactional guard, and react-query's `mutations: { retry: 1 }` can re-fire a money mutation — but whether that actually double-pays depends on whether the payout/refund edge functions are idempotent server-side, which we can't see. Either way, non-atomic read-then-write plus automatic retries is the wrong shape for money.
